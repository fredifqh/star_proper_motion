...
std::vector<double> ConvertDegRAtoRAcoord(double RAinDegree, double DEinDegree)
{	
...
std::vector<double> ProperMotion(double hourRA, double minuteRA, double secondRA, double degreDec,\
	                             double minuteDec, double secondDec, double muAlpha, double muDelta,\
	                             double radialVelocity, double paralaxe, double time)
{
    double	RA = ConvertRAToDegrees(hourRA, minuteRA, secondRA)*PI/180;
    double	DEC = ConvertDecToDegrees(degreDec, minuteDec, secondDec)*PI/180;
    ...
    double RAinDegree = atan2(v[1], v[0])*180/PI;
	...
	double DECinDegree = atan2(v[2], sqrt(pow(v[0],2) + pow(v[1],2)))*180/PI;
